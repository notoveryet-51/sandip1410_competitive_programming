/*The program is the solution of codeforces problem no. 424B
Regn no.: 2025CA085
Program Date: 25-09-2026
Program Time: 00:29 IST    */
...
/*----------------------------------SOLVE FUNCTION----------------------------------*/
void solve() {
    ll n, curr; cin>>n>>curr;
    vector<pair<double, int>> vec;
    while (n--) {
        ll x, y, k; cin>>x>>y>>k;
        double root=sqrt(x*x+y*y);
        vec.push_back({root, k});
    }
    ...
    for (auto it: vec) {
        curr+=it.second;
        if (curr>=1000000) {
            cout<<fixed<<setprecision(7)<<it.first<<endl;
            return;
        }
    }
    cout<<-1<<endl;
