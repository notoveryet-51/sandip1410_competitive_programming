/*The program is the solution of codeforces problem no. 1729D
Regn no.: 2025CA085
Program Date: 17-05-2026
Program Time: 15:33 IST    */
...
void solve() {
    ll n;   cin>>n;
    vll spend(n);    read(spend);
    vll budget(n);  read(budget);
    vpll diff(n);
    for (ll i=0; i<n; i++) {
        diff[i].first=budget[i]-spend[i];
        diff[i].second=i;
    }
    sort(rall(diff));
    ll j=n-1, cnt=0;
