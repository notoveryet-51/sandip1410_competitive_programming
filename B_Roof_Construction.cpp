/*The program is the solution of codeforces problem no. 1632B
Regn no.: 2025CA085
Program Date: 30-05-2026
...
void solve() {
    ll n;   cin>>n;
    // largest power of 2 that is strictly less than n
    ll k=0;
    for (ll i=0; i<n; i++) {
        ll curr=1<<i;
        if (curr<n) k=max(k, curr);
    }
    // print n-1 to k
    for (ll i=n-1; i>=k; i--) {
        cout<<i<<" ";
