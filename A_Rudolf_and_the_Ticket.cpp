/*The program is the solution of codeforces problem no. 1941A
Regn no.: 2025CA085
Program Date: 25-07-2026
...
/*----------------------------------SOLVE FUNCTION----------------------------------*/
void solve() {
    ll n, m, req;   cin>>n>>m>>req;
    vll nums1(n);   read(nums1);
    vll nums2(m);   read(nums2);
    sort(all(nums1));   sort(all(nums2));
    ll count=0;
    for (ll i=0; i<n; i++) {
        for (ll j=0; j<m; j++) {
            if (nums1[i]+nums2[j]<=req) count++;
        }
    }
    cout<<count<<endl;
}
